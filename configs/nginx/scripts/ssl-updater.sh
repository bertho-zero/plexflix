...
# From https://gist.github.com/maxivak/4706c87698d14e9de0918b6ea2a41015
...
le_docker_renew() {
    docker run --rm --name temp_certbot \
        ...
        certbot/certbot certonly --webroot --agree-tos --no-eff-email --renew-by-default \
...
le_native_renew() {
    certbot certonly --webroot --agree-tos --no-eff-email --renew-by-default \
        --config-dir ${CERT_DIR_PATH} \
        --preferred-challenges http-01 \
        --server https://acme-v02.api.letsencrypt.org/directory --text ${ADDITIONAL} \
        --email ${EMAIL_ADDRESS} -w ${WEBROOT_PATH} ${LE_DOMAINS}
}

le_renew() {
    if [[ $NATIVE_CERTBOT -eq 1 ]]; then
        le_native_renew
    else
        le_docker_renew
    fi
